...
| 型 | 定義場所 | 用途 |
|----|----------|------|
| FormInputType | src/components/Form/Field/types.ts | フォーム入力種別 |
| FieldDataType | 同上 | Neon DB データ型 |
| FieldDataTypeFirestore | 同上 | Firestore データ型 |
| FieldConfig | 同上 | domain.json Field 定義 |
...
FieldConfig を拡張したロール別プロフィール用の型。

```typescript
type ProfileFieldConfig<TTag extends string = string> = FieldConfig & {
  fieldType: FieldDataType;      // 必須に上書き
...
```typescript
// domain.json インポート時
const fields = (domainConfig.fields ?? []) as FieldConfig[];
```
...
- src/components/Form/Field/ - フィールド共通型
- src/components/Form/FieldRenderer/ - フォームレンダリング
