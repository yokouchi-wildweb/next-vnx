...
### 2. JWT の発行・検証ポリシー
- JWT は Access Token のみを利用し、`sub`（userId）、`role`、`status`、`isDemo`、`providerType`、`providerUid`、`name`、`expiresAt` などのペイロードを含める。
...
| `src/features/auth/entities/session.ts` | JWT 生成で必須となる `role`・`providerType`・`name` などの属性を含むセッションユーザーとトークンペイロードのスキーマを定義する。 |
