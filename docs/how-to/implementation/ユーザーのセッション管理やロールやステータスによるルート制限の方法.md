...
export default async function MyPage() {
  ...
  return (
    <div>
      <h1>ようこそ、{sessionUser.name} さん</h1>
...
`SessionUser` 型には `userId` / `role` / `status` / `providerType` / `providerUid` / `name` が含まれます。追加のユーザーデータが必要な場合は、この ID を使ってドメインサービスから詳細情報を取得してください。
