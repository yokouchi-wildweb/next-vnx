...
### 2-2. 用語を活用する際のコツ
ここに挙げた名称や機能は MCP がレイヤー構造を把握する際の前提になります。レイヤー名やコンポーネント名をプロジェクト全体で統一し、node-id の把握方法をメンバー間で共有しておくと、以降の設計・実装フローが格段にスムーズになります。

---

## 3. 🛠️ デザイン構築の基本方針と具体的手順

### 3-1. 🧩 セクション単位でフレームを分ける
...
- 複数の図形などを組み合わせた要素はグループに対してエクスポートを設定します。
- できるだけベクター（Outline）で作成する。  
  例: `Pen Tool` や `Rectangle` で描いた図形を組み合わせて、グループに対して右クリック→`Flatten`。
- ラスタ画像を使わざるを得ない場合も、名称とグループ化を行い、エクスポート設定を付けておくと Codex が取得しやすいです。

### 3-7. 🎨 カラースタイル・テキストスタイル
...
- 装飾の都合でアウトライン化が必要な場合はアウトライン化後に画像としてエクスポートします。
