...
| `count(params)` | `CountParams & ExtraWhereOption` | `search` と同じフィルタ条件で `COUNT(*)` のみを実行し `{ total }` を返します。レコードの取得・ソート・ハイドレーションは行いません。 | 件数表示やバッジ等に最適。 |
| `countWithDeleted(params)` | 同上 | `count` の論理削除レコード含む版。 | |
