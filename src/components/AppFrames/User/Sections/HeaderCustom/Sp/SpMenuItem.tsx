...
export const SpMenuItem = ({ item, showIcon = true, onNavigate }: SpMenuItemProps) => {
  ...
  if (hasChildren(item)) {
    ...
    return (
      <div id={`header-custom-sp-accordion-${item.key}`}>
        <div id={`header-custom-sp-accordion-trigger-${item.key}`} className={styles.trigger} onClick={handleToggle}>
          {hasValidHref(item.href) ? (
            <Link
              id={`header-custom-sp-accordion-link-${item.key}`}
              href={item.href}
              onClick={(e) => {
                e.stopPropagation();
                onNavigate?.();
            ...
            <span id={`header-custom-sp-accordion-label-${item.key}`} className="flex-1">
              <MenuItemLabel label={item.label} icon={item.icon} showIcon={showIcon} />
            </span>
          )}
          <ChevronDown
        ...
        <AnimatePresence>
          {isExpanded && (
            <motion.ul
              id={`header-custom-sp-accordion-children-${item.key}`}
            ...
            >
              {item.children?.map((child) => (
                <li key={child.key} id={`header-custom-sp-accordion-child-${child.key}`}>
  ...
  if (isActionItem(item)) {
    ...
    return (
      <Button
        id={`header-custom-sp-action-${item.key}`}
        type="button"
        onClick={handleClick}
  ...
  // 通常のリンク/非リンクアイテム
  return (
    <MenuItemLink id={`header-custom-sp-link-${item.key}`} href={item.href} className={styles.link} onClick={onNavigate}>
      <MenuItemLabel label={item.label} icon={item.icon} showIcon={showIcon} />
    </MenuItemLink>
  );
};
...
const SpMenuItemChild = ({ item, showIcon = true, onNavigate }: SpMenuItemChildProps) => (
  <MenuItemLink id={`header-custom-sp-child-link-${item.key}`} href={item.href} className={styles.childLink} onClick={onNavigate}>
    <MenuItemLabel label={item.label} icon={item.icon} showIcon={showIcon} />
  </MenuItemLink>
);
